...
#Set the margin
par(mar = c(6, 6, 5, 4))
#create the plot without any points, set the labels for x and y axes
plot(epcData$DateTime, epcData$Global_active_power, xlab = "", ylab = "Global Active Power (kilowatts)", type = "n")
#draw the lines
lines(epcData$DateTime, epcData$Global_active_power, type = "S")
#close the device
dev.off()
}
